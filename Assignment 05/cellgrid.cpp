...
int Cellgrid::returnCells(int row, int col, int conn){
    int index = (row * cols + col);
    if(row < 0 || col < 0 || row >= rows || col >= cols || grid[index] == 0){
        return 0;
    }  
    grid[index] = 0;
    if(conn == 4){
        return 1 + returnCells(row, col - 1, conn) + returnCells(row, col + 1, conn) + returnCells(row - 1, col, conn) + returnCells(row + 1, col, conn);
    } else{
        return 1 + returnCells(row, col - 1, conn) + returnCells(row, col + 1, conn) + returnCells(row - 1, col, conn) + returnCells(row + 1, col, conn) + 
        returnCells(row + 1, col + 1, conn) + returnCells(row - 1, col + 1, conn) + returnCells(row + 1, col - 1, conn) + returnCells(row - 1, col - 1, conn);
    }
...
int Cellgrid::countCells(int row, int col, int conn) {
    ...
    row = row - 1;
    col = col - 1;
    int value = returnCells(row, col, conn);
...
int Cellgrid::countBlobs(int conn) {
    ...
    for(int i = 0; i < index; i++){
        if(grid[i] == 1){
        returnCells(i/rows, i%cols, conn);
        blobCount += 1;
        } 
    }
    // this is where your private helper method which does the
